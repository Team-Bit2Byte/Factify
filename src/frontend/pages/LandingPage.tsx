...
import { analyzeImage, analyzeUrl, type AnalysisResult } from '../services/api';
  ...
  const handleAnalyze = useCallback(async () => {
    if (activeTab === 'image' && selectedFile) {
      ...
      try {
        ...
        setUploadError(error instanceof Error ? error.message : 'Failed to analyze URL. Please make sure the backend server is running.');
      } finally {
        setIsUploading(false);
      }
    } else {
      // Text analysis is still mock-only.
      setShowAnalysis(true);
    }
  }, [activeTab, selectedFile, urlInput]);

  return (
    <div className="min-h-screen bg-gradient-to-b from-surface via-surface-container-low to-slate-100">
      <main className={`pt-20 pb-10 mx-auto grid grid-cols-1 gap-6 px-4 sm:px-6 md:pt-24 md:pb-12 lg:grid-cols-12 lg:gap-10 xl:gap-12 transition-all duration-700 ${showAnalysis ? 'max-w-[1700px] w-full' : 'max-w-4xl'}`}>
        {/* Left Content: Hero & Input */}
        <div className={`space-y-8 md:space-y-10 lg:space-y-12 transition-all duration-700 ${showAnalysis ? 'lg:col-span-5 xl:col-span-4' : 'lg:col-span-12'}`}>
          {/* Hero Section */}
            ...
            <div className="space-y-6 p-5 sm:p-6 md:p-8">
                    ...
                    >
                      {selectedFile ? (
                        <>
                          ...
                          <p className="mb-1 break-words text-base font-bold text-on-surface sm:text-lg">{selectedFile.name}</p>
                        ...
                        <>
                          ...
                          <p className="mb-1 text-base font-bold text-on-surface sm:text-lg">Drag and drop an image, or click to browse</p>
                          <p className="text-sm leading-6 text-secondary">Supports JPG, PNG, WEBP, and AVIF for advanced metadata forensics.</p>
                        </>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-4 pt-4 sm:gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:gap-4">
                  <button type="button" onClick={() => alert('Language auto-detected: English')} className="flex items-center gap-2 text-left text-sm font-medium text-secondary hover:text-primary transition-colors">
                    <Sparkles className="w-4 h-4" />
                    Auto-detect Language
                  </button>
                  <button type="button" onClick={() => alert('Context Tuning options open')} className="flex items-center gap-2 text-left text-sm font-medium text-secondary hover:text-primary transition-colors">
                ...
                <button 
                  ...
                  disabled={isUploading || (activeTab === 'image' && !selectedFile) || (activeTab === 'url' && !urlInput.trim())}
