...
app.post("/api/analyze-text", async (req, res) => {
  ...
  if (submittedText.length > 100000) {
    return res
      .status(413)
      .json({ error: "Text too large (max 100,000 characters)" });
  }
...
function summarizePythonError(stderrData) {
  ...
  if (
    clean.includes("Tesseract OCR is not installed") ||
    clean.includes("TesseractNotFoundError")
  ) {
  ...
  return new Promise((resolve, reject) => {
    ...
    pythonProcess.stderr.on("data", (data) => {
      ...
      if (stderrData.length > MAX_OUTPUT_SIZE) {
        console.error(
          "[Python] Error output size limit exceeded - killing process",
        );
        pythonProcess.kill("SIGTERM");
        cleanup();
        reject(
          new Error("Python script error output exceeded size limit (50MB)"),
        );
      }
    });
    ...
    pythonProcess.on("close", (code, signal) => {
      ...
      if (code !== 0) {
        ...
        return reject(
          new Error(`Python script failed with ${exitDetail}: ${summary}`),
        );
      }
      ...
      try {
        ...
        reject(
          new Error(`Failed to parse Python script output: ${error.message}`),
        );
      }
    });
...
app
  .listen(PORT, "0.0.0.0", () => {
    ...
    console.log(
      `📝 Text analysis endpoint: http://localhost:${PORT}/api/analyze-text`,
    );
    console.log(
      `📤 Image upload endpoint: http://localhost:${PORT}/api/analyze-image`,
    );
    console.log(
      `🔗 URL scrape endpoint: http://localhost:${PORT}/api/analyze-url\n`,
    );
    if (hasBuiltFrontend) {
